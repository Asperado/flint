...
# replacing 'import mimetools' with the following imports - jheinly
import socket
import random
import time
import os
...
class FlickrAPI:
    ...
    def __getattr__(self, method, **arg):
        ...
        if not method in self.__handlerCache:
            def handler(_self = self, _method = method, **arg):
                ...
                #print "--postData----------------------------------------"
                #print postData
                f = urllib.request.urlopen(url, postData.encode('utf-8'), timeout=30)
                data = f.read()
                # NOTE: this was added to remove invalid unicode characters from the string - jheinly
    ...
    def upload(self, filename=None, jpegData=None, **arg):
        ...
        # construct POST data
        # mimic the mimetools.choose_boundary() call - jheinly
        ipAddress = socket.gethostbyname(socket.gethostname())
        currentPID = str(os.getpid())
        timestamp = str(time.time())
        randomVal = str(random.random())
        boundary = ipAddress + '.' + currentPID + '.' + timestamp + '.' + randomVal
    ...
    #-----------------------------------------------------------------------
    #@classmethod
    # return True on failure, and false otherwise
